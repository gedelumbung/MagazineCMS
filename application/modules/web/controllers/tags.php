...
class tags extends CI_Controller {
	...
	function index($id_param=0,$slug="",$offset=0)
	{
		...
		//breadcrumb
		$get_tags = $this->db->get_where("dlmbg_tags",array("id_tags"=>$id_param))->row();
		$this->breadcrumb->append_crumb('Beranda', base_url());
		$this->breadcrumb->append_crumb("Tags : ".$get_tags->tags, base_url().'web/bidang');

		$d['meta_description'] = $GLOBALS['site_meta'];
		$d['meta_keywords'] = $GLOBALS['site_keywords'];
		$d['meta_author'] = $GLOBALS['site_author'];
		$d['title'] = 'Tags : '.$get_tags->tags.' - ';
		...
		//content
		$d['list_berita'] = $this->app_global_web->generate_berita_list_thumb_tags($id_param,$slug,$offset,"DESC",$GLOBALS['site_limit_medium']);
		...
		$d['kategori_footer'] = $this->app_global_web->generate_kategori_footer();
 		...
 		$this->load->view($GLOBALS['site_theme']."/front/tags/bg_home");
